...
#Subsetting the data
#a_users <- arrange(summarise(group_by(user_artists, artistID), 
...
#Item based collaborative filtering for single user
u1 = '6'

tm <- matrix(, nrow=1, ncol=ncol(test_data), dimnames=(list(list(u1), colnames(test_data))))
tm[1, ] <- t(as.matrix(train_data[u1,]))

ResultsIBCFUser <- ItemBasedCF_pearson(train_data, tm, 3, NN=10, onlyNew=TRUE)

predictionUserone <- as.data.frame(ResultsIBCFUser$prediction)
predictionUserone

TopNUserone <- as.data.frame(ResultsIBCFUser$topN)
TopNUserone

#using cosine in item based for single user
ResultsIBCFUser_cosine <- ItemBasedCF(train_data, tm, 3, NN=10, onlyNew=TRUE)

predictionUserone_cosine <- as.data.frame(ResultsIBCFUser_cosine$prediction)

TopNUserone_cosine <- as.data.frame(ResultsIBCFUser$topN)
TopNUserone_cosine
